...
void main()
{
  ...
  if (gWorldBuffer.enableAA >= 1) {
    vec2 aaCoord = frag_in.uv * vec2(gWorldBuffer.screenSize);
    color = AntiAliasingFXAA(sceneSurface, aaCoord, gWorldBuffer.screenSize).rgb;
  } else {
     color = texture(sceneSurface, frag_in.uv).rgb;
  }
